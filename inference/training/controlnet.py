...
class ControlNet(nn.Module):
  ...
  def __init__(
      self, use_5b_model=False, use_vggt_latents=False, vggt_condition='seq'
  ):
    super().__init__()
    ...
    pose_channels = [
        ...
        384,
        48,
    ]  # 48 is the channel dim of 5B VAE; if I choose to do seq conditioning
    ...
    if use_vggt_latents:
      ...
      self.vggt_latents_spatial_conv2 = nn.Sequential(
          nn.Conv2d(
              latents_channels, out_channels, kernel_size=1, stride=1, padding=0
          ),
  ...
  def preprocess_vggt_latents(self, vggt_latents, target_shape):
    ...
    vggt_latents = F.interpolate(
        vggt_latents, scale_factor=0.5, mode='bilinear', align_corners=False
    )
    vggt_latents = rearrange(
        vggt_latents, '(b f) c h w -> b c f h w', b=b
    ).contiguous()
    ...
    vggt_latents = F.interpolate(
        vggt_latents, size=target_shape, mode='bilinear', align_corners=False
    )
    vggt_latents = rearrange(
        vggt_latents, '(b f) c h w -> b c f h w', b=b
    ).contiguous()
