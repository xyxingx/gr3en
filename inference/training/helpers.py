...
def save_image_paths_to_files(
    image_paths, path_name, output_dir="./saved_paths"
):
...
def save_video_as_gif(
  ...
  Args:
      ...
      duration (Optional[int]): Duration per frame in milliseconds. If None,
        calculated from fps.
      input_range (Literal["0_1", "-1_1"]): Input value range. "0_1" for [0,1],
        "-1_1" for [-1,1] (default: "0_1")
  ...
  if video_tensor.dim() != 5:
    raise ValueError(
        f"Expected 5D tensor (B, N, 3, H, W), got {video_tensor.dim()}D"
    )
  ...
  if input_range not in ["01", "-11"]:
    raise ValueError(
        f"input_range must be '0_1' or '-1_1', got '{input_range}'"
    )
  ...
  if input_range == "01":
    ...
    if video_tensor.min() < -0.1 or video_tensor.max() > 1.1:
      print(
          "Warning: Tensor values outside [0,1] range. Min:"
          f" {video_tensor.min():.3f}, Max: {video_tensor.max():.3f}"
      )
    ...
    if video_tensor.min() < -1.1 or video_tensor.max() > 1.1:
      print(
          "Warning: Tensor values outside [-1,1] range. Min:"
          f" {video_tensor.min():.3f}, Max: {video_tensor.max():.3f}"
      )
  ...
  print(
      f"GIF saved to: {output_path} ({N} frames, {fps} fps, input_range:"
      f" {input_range})"
  )
...
def save_images_as_grid(imgs, fixed_height=256, spacing=5, max_per_row=5):
  ...
  for np_img in imgs:
    ...
    if len(current_row) < max_per_row:
      ...
      row_widths.append(
          sum(img.width for img in current_row)
          + spacing * (len(current_row) - 1)
      )
  ...
  if current_row:
    row_widths.append(
        sum(img.width for img in current_row) + spacing * (len(current_row) - 1)
    )
    row_images.append(current_row)

  total_width = max(row_widths)
  total_height = fixed_height * len(row_images) + spacing * (
      len(row_images) - 1
  )
...
def torch_batch_to_np_arr(batch, assume_neg1_pos1=False):
  ...
  Args:
      ...
      assume_neg1_pos1: if True, assumes input is in [-1,1] range and uses 127.5
        * x + 128 conversion if False, normalizes input to [0,1] based on
        min/max values
  """
  np_imgs = []
  ...
  if assume_neg1_pos1:
    batch = (
        torch.clamp(127.5 * batch + 128.0, 0, 255)
        .float()
        .cpu()
        .numpy()
        .transpose(0, 2, 3, 1)
...
def save_gifs_as_grid(
  ...
  assert (
      len(pred_frames) == n_frames
  ), "Video and prediction frames must have same length"
  if gt_frames is not None:
    assert (
        len(gt_frames) == n_frames
    ), "Ground truth frames must have same length"
  ...
  grid_img = save_images_as_grid(
      np_frames, fixed_height=fixed_height, spacing=spacing, max_per_row=1
  )
...
class CoordinateMap:
  ...
  def plot_trajectory(
    ...
    if show_all:
      ax.scatter(
          self.xy[:, 0], self.xy[:, 1], s=12, c="lightgray", label="all poses"
      )
