...
def color_region_masks_palette(
    img_seq,
    *,
    specs=('110', '101', '011', '111', '11h', '1h1', 'h11', 'hh1', 'h1h', '1hh'),
    hval=0.5,
    intensity_thr=0.30,
    ...
    """
    assert img_seq.ndim == 4 and img_seq.shape[-1] == 3, "img_seq must be (T,H,W,3)"
    ...
    for spec, target in palette.items():
        diff = np.abs(img_seq - target[None, None, None, :])  # broadcast to (T,H,W,3)
        match = (diff <= float(tol_per_channel)).all(axis=-1)  # per-pixel & per-channel
...
def fourier_embed(x, n_freqs=12):
    ...
    If x is scalar, returns (2*n_freqs,) tensor.
    If x is (d,), returns (d, 2*n_freqs) tensor.
    If x is (..., d), returns (..., d, 2*n_freqs) tensor.
    """
...
def _load_and_process_image(
    ...
    **kwargs,
    ...
    image = np.array(
        Image.open(open(image_path, 'rb')).convert('RGB')
    ) / 255.0
...
class GR3EN_dataset(Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            start_idx: Fixed start index for sampling frames; if None, random
              start.
        ...
        self.random_light_specs = tuple(random_light_specs) if random_light_specs else tuple()
        ...
        default_specs = (
            '110', '101', '011', '111',
            '11h', '1h1', 'h11', 'hh1', 'h1h', '1hh',
            '001', '010', '100',   # pure B, G, R
        )
        ...
        if self.use_5b_model:
            ...
            self.pose_hw = (44, 80)   # 5b model VAE stride=16
        else:
            image_size = (480, 832)
            self.pose_hw = (60, 104)
        ...
        self.dataset = sorted(
            [f for f in sorted(os.listdir(self.mask_dir)) if _is_image(f)]
        )
        if 0 < len(self.dataset) < self.sample_n_frames:
            last_frame = self.dataset[-1]
            self.dataset.extend([last_frame] * (self.sample_n_frames - len(self.dataset)))
            self.frame_step = 1
        self.length = max(
            0,
        ...
        print(
            ...
            f"vary_intensity={self.vary_intensity}, intensity_min={self.intensity_min}, "
            f"variation_steps={self.variation_steps}, vary_color={self.vary_color}, "
            f"color_target={self.color_target}"
        )
    ...
    def __getitem__(self, idx):
        ...
        abti_tensor_base = fourier_embed(abti_tensor[0])
        ...
        # ----------- Torch & normalization -----------
        pixel_values = torch.from_numpy(video_frames).permute(0, 3, 1, 2).contiguous()
        relit_pixels = torch.from_numpy(relit_video_frames).permute(0, 3, 1, 2).contiguous()
        ...
        video_captions = (
            "a photorealistic fisheye video of an indoor room, the lamp is turned off, "
            "camera is rotating around the room to capture a 360 degree view of the room"
        )
