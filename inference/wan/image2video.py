...
from training import flow_match_scheduler
...
class WanI2V(nn.Module):
  ...
  def __init__(
    ...
    r"""Initializes the image-to-video generation model components.
    ...
    if not training:
      ...
      self.dit = WanModel.from_pretrained(checkpoint_dir, subfolder=subfolder)
    ...
    # to process poses
    self.controlnet = controlnet.ControlNet().to(
  ...
  def _prepare_model_for_timestep(self, t, boundary, offload_model):
    r"""Prepares and returns the required model for the current timestep.

    Args:
        t (torch.Tensor): current timestep.
        boundary (`int`): The timestep threshold. If `t` is at or above this
          value, the `high_noise_model` is considered as the required model.
        offload_model (`bool`): A flag intended to control the offloading
          behavior.
  ...
  def save_checkpoint(
    ...
    if self.latent_dpt is not None:
      latent_dpt_state_dict = get_model_state_dict(
          model=self.latent_dpt, options=options
      )
  ...
  def diffusion_forward(self, batch, overwrite_timesteps=None):
    ...
    if self.low_model_or_high_model == 'low':
      max_timestep_boundary = 0.9
      min_timestep_boundary = 0
    if self.low_model_or_high_model == 'high':
      max_timestep_boundary = 1
      min_timestep_boundary = 0.9
    max_timestep_boundary = int(
        max_timestep_boundary * self.scheduler.num_train_timesteps
    )
    min_timestep_boundary = int(
        min_timestep_boundary * self.scheduler.num_train_timesteps
    )
    ...
    noisy_model_input, timestep = self.scheduler.add_noise(
        video_latents, noise, timestep
    )
  ...
  def prepare_batch(self, batch, device, dtype=torch.bfloat16):
    ...
    vae_input = torch.cat(
        [first_image, first_image.new_zeros((b, f - 1, c, h, w))], dim=1
    )
    ...
    msk = torch.ones(
        1, f, pose_h, pose_w, device=self.device, dtype=first_image_latent.dtype
    )  # pose_hw = latent_hw
    msk[:, 1:] = 0
    msk = torch.cat(
    ...
    first_image_latent = torch.cat(
        [msk, first_image_latent], dim=1
    )  # B,20,f,h,w
  ...
  def run_one_step(self, batch, noisy_model_input, timestep):
    model_out = self.dit(
        ...
        self.training,
        poses=batch['poses'],
        pose_proj=self.controlnet.pose_proj
        if self.controlnet is not None
        else None,
    )
  ...
  @torch.inference_mode
  def vis_training(
      self, ref, model_out, noisy_model_input, step, timesteps, outdir
  ):
  ...
  def generate(
    ...
    Args:
        ...
        max_area (`int`, *optional*, defaults to 720*1280): Maximum pixel area
        ...
        shift (`float`, *optional*, defaults to 5.0): Noise schedule shift
          parameter. Affects temporal dynamics [NOTE]: If you want to generate a
          480p video, it is recommended to set the shift value to 3.0.
        sample_solver (`str`, *optional*, defaults to 'unipc'): Solver used to
          sample the video.
        sampling_steps (`int`, *optional*, defaults to 40): Number of diffusion
        ...
        guide_scale (`float` or tuple[`float`], *optional*, defaults 5.0):
          ...
          creativity. If tuple, the first guide_scale will be used for low noise
          model and the second guide_scale will be used for high noise model.
    ...
    if data_batch is None:
      ...
      h, w = 480, 832
      lat_h, lat_w = 60, 104
      poses = data_batch['poses'][None].to(
    ...
    context = torch.load('prompt_embed.pt').to(
        self.device, dtype=torch.bfloat16
    )
    context_null = torch.load('null_prompt_embed.pt').to(
        self.device, dtype=torch.bfloat16
    )
    context = [context]
