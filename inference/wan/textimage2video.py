...
class WanTI2V(nn.Module):
    ...
    def load_checkpoint(self, checkpoint_path, optimizer=None, lr_scheduler=None, is_main=False):
    ...
    def load_sharded_checkpoint(self, checkpoint_path, global_step, optimizer=None, lr_scheduler=None):
        '''
    ...
    def diffusion_forward(self, batch, overwrite_timesteps=None):
        ...
        max_timestep_boundary = int(batch.get("max_timestep_boundary", 1) * self.scheduler.num_train_timesteps)
        min_timestep_boundary = int(batch.get("min_timestep_boundary", 0) * self.scheduler.num_train_timesteps)
        noise_sample_prob = random.random()
        if noise_sample_prob > 0.15:
            timestep_id = torch.randint(min_timestep_boundary, int(max_timestep_boundary * 0.4), (1,))
        else:
            timestep_id = torch.randint(int(max_timestep_boundary * 0.4), max_timestep_boundary, (1,))
        ...
        noisy_model_input = self.scheduler.add_noise(batch['video_latents'], noise, timestep)
    ...
    def prepare_batch(self, batch, device, dtype=torch.bfloat16):
        ...
        if cfg_p < 0.12 and self.enable_cfg:
            input_video_latents = torch.cat([input_video_latents, content_mask], dim=2)
        else:
            # add the mask as an extra condition
            input_video_latents = torch.cat([input_video_latents, masks_latnents], dim=2)
    ...
    @torch.inference_mode
    def vis_training(self, ref, model_out, noisy_model_input, step, timesteps, outdir, inputs):
        ...
        input_frames = video_processor.postprocess_video(video=input_frames, output_type='pil')[0]  # N, H, W, 3
        ...
        out_reference_frames = [
            functions.stack_images_horizontally(frame_input, frame_reference, frame_out)
            for frame_input, frame_out, frame_reference in zip(
                input_frames, video, reference_frames
            )
        ]
        media.write_video(
            output_path_file_out_reference,
            np.array(out_reference_frames),
            fps=10
        )
    ...
    def v2v(
        ...
        condition = data_batch['videos_input'].to(self.device, torch.bfloat16).transpose(0, 1)
        ...
        ambient_embed = data_batch['abt_embed'].to(self.device, torch.bfloat16)
        ambient_embed = ambient_embed[None].repeat(1, 3, 1)
        ae_embed = data_batch.get('ae_embed', None)
        ...
        with (torch.amp.autocast('cuda', dtype=self.param_dtype), torch.no_grad()):
            for t in timesteps:
                ...
                new_lat = sample_scheduler.step(
                    eps.unsqueeze(0), t, latent.unsqueeze(0),
