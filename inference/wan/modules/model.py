...
class WanModel(ModelMixin, ConfigMixin):
    ...
    def forward(self, x, t, context, seq_len, y=None, is_training=False, **regr_kwargs):
        device = self.patch_embedding.weight.device
        if self.freqs.device != device:
            self.freqs = self.freqs.to(device)
        ...
        xy = torch.cat([x, y], dim=2) if y is not None else x  # [B, C, F_x+F_y, H, W]

        # ---- patchify ----
        x = self.patch_embedding(xy)              # [B, D, F_p, H_p, W_p] where F_p = (F_x+F_y)/t_patch
        grid_sizes = torch.tensor(x.shape[2:], dtype=torch.long, device=x.device)[None]  # [1,3]=(F_p,H_p,W_p)
        ...
        F_total_p = int(grid_sizes[0, 0])
        assert F_total_p == F_x_p + F_y_p, "Frame accounting mismatch after patching"
        ...
        e  = self.time_embedding(
            sinusoidal_embedding_1d(self.freq_dim, t.flatten()).unflatten(0, (B, L_total)).to(dtype=torch.bfloat16)
        )                                            # [B, L, C]
        e0 = self.time_projection(e).unflatten(2, (6, self.dim))  # [B, L, 6, C]
    ...
    def original_forward(
        ...
        if y is not None:
            x = [torch.cat([u, v], dim=0) for u, v in zip(x, y)] # e.g. x[0].shape now = [36, 21, 64, 96]


        # embeddings
        x = [self.patch_embedding(u.unsqueeze(0)) for u in x] # patch embedding: conv3d 36->5120 (doesn't flatten, changes channel and halves spatial dims->5120,21,32,48)
